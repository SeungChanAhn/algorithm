...
class Gongsa {
	...
	public boolean isSilent(Tree tree) {
		return Math.pow(tree.x - x, 2) + Math.pow(tree.y - y, 2) >= Math.pow(R, 2);
	}
...
class Park {
	...
	public List<Boolean> getSilentResult() {
		...
		for (Tree tree : trees) {
			boolean result = Math.pow(gongsa.x - tree.x, 2) + Math.pow(gongsa.y - tree.y, 2) >= Math.pow(gongsa.R, 2);
